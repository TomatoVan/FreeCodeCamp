...
function decrypt(encryptedText, n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let evenCount = 0;
    let oddCount = 0;
    for (let k = 1; k < encryptedText.length + 1; k++) {
      if (k % 2 == 0) {
        evenMass[evenCount] = encryptedText[k];
        evenCount++;
        console.log(evenMass);
      }
      else {
        oddMass[oddCount] = encryptedText[k];
        oddCount++;
        console.log(oddMass);
      } 
    }
    encryptedText == [...evenMass, ...oddMass].join('');
